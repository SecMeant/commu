...
void ChatWindow::sendMSG()
{
		...
		// Client want to send private msg
		if(msg.startsWith("/pw "))
		{
			QString pwUser;
			msg.remove(0,4);
			for( int i = 0; i < msg.size() && msg[i] != ' '; ++i )
			{
				pwUser += msg[i];
			}
			msg.remove(0,pwUser.size());
			mlp.fillFrame('6',pwUser.toLocal8Bit().constData(),msg.toLocal8Bit().constData());
